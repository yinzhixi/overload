...
<form class="layui-form" action="" style="padding: 20px 10px;width: 90%;text-align: center;">
  <div class="layui-form-item">
    ...
    layui.use(['laypage','jquery','form'], function(){
          ...
          //分页查询
          pageQuery = function () {
              $.get("${contextPath}/flowView/byCarNum/queryAllReverse",{page:page.page,limit:page.pageSize, 
            	  stationName:$("#station").val(),carNum:$("#carNum").val()},
            	  function(res){
                      renderItems(res.data.list);
                      count = res.count;
                      resetPage(res.count, page.page,false);
                   /*   t=setTimeout("pageQuery()",3000); */
            	  }
              );
          }
        ...
        //重置分页(跳转分页)
          ...
          renderItems = function (items,append){
              ...
              $.each(items,function(i,item){
            	  var url = encodeURI('/static/viewflow/?fileName='+item.frontPic);
                   var itemHtm = [
                       '<div class="flex-cell">',
                       ' <div class="img-wrap">',
                       '     <img alt="" id="'+item.id+'" src="'+url+'">',
                       ' </div>',
                        '<div class="img-info">',
                        '    <div>',
                        '    <span class="lb">'+(!item.carNum?'':item.carNum)+'</span> &nbsp;&nbsp;',
                        '    <span class="lb">车道</span>&nbsp;'+(!item.lane?'':item.lane)+'&nbsp;&nbsp;',
                        '    <span class = "ss">'+(!item.snapTime?'':item.snapTime)+
                    	'</span>',
                        '    </div>',
                        '</div>',
                    '</div>'
    	        ...
    	        if(typeof(event.data) == "string"){
    	        	 var obj = JSON.parse(event.data);
       	            if(obj.type=='snap'){
       	            	...
       	            	item.imgUrl = encodeURI("${staticPath}?fileName="+item.frontPic+"?_p="+Math.random());
